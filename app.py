...
ROLE_CHOICES = ("Admin", "Officer", "Resident")
...
CREATE TABLE IF NOT EXISTS users (
    ...
    role TEXT NOT NULL CHECK (role IN ('Admin', 'Officer', 'Resident')),
...
def fetch_lookups(connection: sqlite3.Connection) -> dict:
    ...
    centers = fetch_centers(connection)
    return {
        "communities": [{"id": item["id"], "label": item["label"]} for item in communities],
        "organizations": [
            {"id": item["id"], "label": item["organization_name"], "type": item["type"]}
            for item in organizations
        ],
        "disasters": [
            {
                "id": item["id"],
                "label": f"{item['disaster_type']} - {item['date_occurred']} ({item['community_name']})",
            }
            for item in disasters
        ],
        "centers": [{"id": item["id"], "label": item["center_name"]} for item in centers],
...
def seed_database(connection: sqlite3.Connection) -> None:
    ...
    users = [
        ...
        (
            community_id,
            "Laica Jene B. Zamudio",
            "resident",
            hash_password("resident123"),
            "Resident",
            "resident@sanisidro.local",
        ),
    ]
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def ensure_write_access(self, current_user: dict, resource: str) -> None:
        if current_user["role"] == "Resident":
            raise ApiError(403, "Residents can only view records in this prototype.")
