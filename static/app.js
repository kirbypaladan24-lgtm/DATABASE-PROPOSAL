  ...
  const database = {
    ...
    users: [
      ...
      },
      {
        id: 3,
        community_id: 1,
        full_name: "Laica Jene B. Zamudio",
        username: "resident",
...
function loadDatabase() {
  ...
  try {
    ...
    if (migrateHistoricalBicolTyphoonSeed(parsed)) {
      saveDatabase(parsed);
    }
    return parsed;
  } catch (error) {
...
function normalizeRole(value) {
  ...
  if (!["Admin", "Officer", "Resident"].includes(role)) {
    throw makeError("role must be one of: Admin, Officer, Resident.");
  }
  return role;
}
...
function buildUsers(database) {
  const roleOrder = { Admin: 1, Officer: 2, Resident: 3 };
...
function buildLookups(database) {
  ...
  return {
    communities: buildCommunities(database).map((item) => ({ id: item.id, label: item.label })),
    ...
    disasters: buildDisasters(database).map((item) => ({
      id: item.id,
      label: `${item.disaster_type} - ${item.date_occurred} (${item.community_name})`,
    })),
    centers: buildCenters(database).map((item) => ({ id: item.id, label: item.center_name })),
    roles: ["Admin", "Officer", "Resident"],
...
function buildTextExport(exportData) {
  ...
  exportData.emergency_contacts.forEach((item) => {
    lines.push(
      ...
      `Community: ${item.community_label}`,
      `Phone Numbers: ${item.phone_numbers.map((phone) => `${phone.network} ${phone.phone_number}`).join(" | ")}`,
      ""
    );
  });
  ...
  exportData.evacuation_centers.forEach((item) => {
    lines.push(
      ...
      `Center Contacts: ${item.center_contacts
        .map((contact) => `${contact.contact_name} (${contact.phone_number})`)
        .join(" | ")}`,
      ""
    );
  });
...
function buildErdTextExport() {
  ...
  const lines = [
    ...
    "- Role must be Admin, Officer, or Resident.",
...
function ensureWriteAccess(user, resource) {
  if (user.role === "Resident") {
    throw makeError("Residents can only view records in this prototype.", 403);
  }
}
...
function handleLocalApi(database, method, path, payload) {
  if (path === "/api/login" && method === "POST") {
    ...
    if (user.role === "Resident") {
      throw makeError("Residents can use the public view. Admin login is for Admins and Officers only.", 403);
    }
...
function renderContacts() {
  ...
  elements.content.contacts.innerHTML = items
    .map(
      (item) => `
        <article class="record-card">
          <div class="record-card-header">
          ...
          <p class="meta-line">${escapeHtml(item.email)}</p>
          <div class="chip-row">
            ${item.phone_numbers
              .map(
                (number) => `<div class="chip"><strong>${escapeHtml(number.network)}</strong>${escapeHtml(number.phone_number)}</div>`
              )
              .join("")}
          </div>
          ${
...
function renderDisasters() {
  ...
  elements.content.disasters.innerHTML = items
    .map(
      (item) => `
        <article class="record-card">
          <div class="record-card-header">
            <div>
              <h3>${escapeHtml(item.disaster_type)}</h3>
              <p class="meta-line">${escapeHtml(item.community_label)} | ${escapeHtml(formatDate(item.date_occurred))}</p>
            </div>
            ${badgeMarkup(item.severity_level, `severity-${item.severity_level.toLowerCase()}`)}
          </div>
          <p>${escapeHtml(summarizeText(item.description, 220))}</p>
          ${
            hasManageAccess()
              ? `<div class="inline-actions">
                  ${actionButton("Edit Disaster", "edit-disaster", item.id)}
                  ${actionButton("Delete Disaster", "delete-disaster", item.id)}
                  ${actionButton("Add Announcement", "add-announcement", item.id)}
                </div>`
              : ""
          }
            ...
            ${
              item.announcements.length
                ? item.announcements
                        ...
                        <div class="record-card">
                          <div class="record-card-header">
                            <div>
                              ...
                              <p class="meta-line">Issued ${escapeHtml(formatDate(announcement.date_issued))}</p>
                            </div>
                            ${badgeMarkup(announcement.alert_level, `alert-${announcement.alert_level.toLowerCase()}`)}
                          </div>
                          <p>${escapeHtml(summarizeText(announcement.message, 180))}</p>
                          ${
                            hasManageAccess()
                              ? `<div class="inline-actions">
                    ...
                    .join("")
                : emptyState("No announcements linked to this disaster yet.")
            }
          </div>
        </article>
      `
...
function renderCenters() {
  ...
  elements.content.centers.innerHTML = items
    .map(
      (item) => `
        <article class="record-card">
          <div class="record-card-header">
            <div>
              <h3>${escapeHtml(item.center_name)}</h3>
              <p class="meta-line">${escapeHtml(item.location)}</p>
            </div>
            <div class="badge">Capacity ${formatNumber(item.capacity)}</div>
          </div>
          <p class="meta-line">${escapeHtml(item.community_label)}</p>
          <div class="contact-list">
            ${item.center_contacts
              .map(
                (contact) => `
                  <div class="chip">
                    <strong>${escapeHtml(contact.contact_name)}</strong>${escapeHtml(contact.phone_number)}
                  </div>
                `
              )
              .join("")}
...
function openUserModal(item = null) {
  ...
  showModal({
    ...
    formHtml: `
      ...
      ${selectField({ name: "role", label: "Role", options: roleOptions(), value: item?.role || "Resident" })}
